...
export default function SignupPage() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await createChurchSignup({ churchName, slug, email });
      setSent(true);
    } catch (err) {
      ...
      <div style={{ width: 380 }}>
        ...
        {sent ? (
          <div style={{ textAlign: "center" }}>
            <p style={{ fontSize: 14, color: "#3A342B" }}>Check your inbox — click the link to finish setting up <strong>{churchName}</strong>.</p>
          </div>
        ) : (
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              <label style={{ display: "block", fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: "#6B6355", fontWeight: 600, marginBottom: 6 }}>Church Name</label>
              <input required value={churchName} onChange={(e) => onNameChange(e.target.value)} placeholder="Grace & Table Church" style={fieldStyle} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: "#6B6355", fontWeight: 600, marginBottom: 6 }}>Your App URL</label>
            ...
            <div>
              <label style={{ display: "block", fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: "#6B6355", fontWeight: 600, marginBottom: 6 }}>Your Email</label>
